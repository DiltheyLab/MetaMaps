...
foreach my $resultsSet (@resultsSets)
{
	...
	my %results_distribution = (
		...
		'Kraken' => $kraken_results_dir . '/results_kraken.txt.ignoreUnclassified',
		'Bracken' => $kraken_results_dir . '/results_bracken.txt.ignoreUnclassified',
	);

	die Dumper("Missing files", [grep {not -e $_} values %results_readLevel]) unless(all {-e $_} values %results_readLevel);
	die Dumper("Missing files", [grep {not -e $_} values %results_distribution]) unless(all {-e $_} values %results_distribution);
